...
class PDG2SeqCell(nn.Module):  #这个模块只进行GRU内部的更新，所以需要修改的是AGCN里面的东西
    ...
    def forward(self, x, state, node_embeddings):
        ...
        # node_embeddings[0] = node_embeddings[0][:, 0, :]
        nodevec1 = torch.tanh(torch.mul(node_embeddings[0], filter1))  #[B,N,dim_in]
        nodevec2 = torch.tanh(torch.mul(node_embeddings[1], filter2))  # [B,N,dim_in]
        # nodevec1 = torch.tanh(node_embeddings[0])  #[B,N,dim_in]
        # nodevec2 = torch.tanh(node_embeddings[1])  # [B,N,dim_in]
        # nodevec1 = filter1  #[B,N,dim_in]
        # nodevec2 = filter2  # [B,N,dim_in]
        ...
        # adj1 = torch.matmul(nodevec1, nodevec2.transpose(2, 1)) - torch.matmul(
        #     nodevec2, nodevec1.transpose(2, 1))
        # adj2 = torch.matmul(nodevec2, nodevec1.transpose(2, 1)) - torch.matmul(
        #     nodevec1, nodevec2.transpose(2, 1))
        # adj1 = DDGCRNCell.preprocessing(F.relu(adj1))
        # adj2 = DDGCRNCell.preprocessing(F.relu(adj2))
